 ...
 * ?????????? v0.7.8 Fix issue with disable not being called when the destroy method is called on a DisplayObject. Allow ComponentFactory.create to be called multiple times with the same selector names and not overwrite active components. Add BulkLoader, ImageLoader ...
